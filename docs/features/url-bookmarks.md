...
- 用户先创建站点，再在站点下添加书签
- 添加书签时，系统按域名自动匹配对应站点
- **添加书签时，若域名无对应站点 → 拒绝创建书签，提示用户先创建站点**
...
左侧为站点列表，按 `updated_at` 降序排列，每条显示站点名称和书签数量。

点击站点后，右侧详情面板分为上下两部分：

- **上半部分**：站点信息（标题、URL、描述、标签、图标等）
- **下半部分**：该站点下所有书签，支持 **网格视图** 和 **列表视图** 切换
  - 网格视图：每个书签显示为卡片，用第一张可预览的图片/视频附件作为封面；无附件时显示标题文字卡片
  - 列表视图：每个书签显示为一行，包含标题、URL 等摘要信息

点击某个书签卡片 → 右侧切换为该书签的完整详情（标题、URL、描述、标签、附件等），提供返回按钮回到站点视图。

```text
🔍 [搜索...]  [标签筛选 ▼]  [+ 新建站点]
┌──────────────────────┬──────────────────────────────────┐
│ GitHub         (3)   │  🌐 GitHub                       │
│ React 官网     (5)   │  https://github.com              │
│ MDN            (2)   │  Tags: #开发 #工具::代码托管       │
│                      │  代码托管平台                      │
│                      ├──────────────────────────────────┤
│                      │  [网格] [列表]                    │
│                      │ ┌──────┐ ┌──────┐ ┌──────┐      │
...
搜索同时匹配站点和书签（平级），但展示时自动将同一站点下命中的结果合并为一张卡片。

- 左侧搜索结果列表：每条显示站点名称 + 命中数量（如"GitHub (2 条命中)"）
- 仅站点本身命中时也显示为一条结果
- 点击搜索结果 → 右侧展示与默认浏览相同的站点+书签视图，书签范围为搜索命中的那些
- 搜索清空后恢复默认站点列表
...
┌──────────────────────┬──────────────────────────────────┐
│ GitHub       (2命中) │  🌐 GitHub                       │
│ Go by Example(1命中) │  https://github.com              │
│                      │  Tags: #开发 #工具                │
│                      ├──────────────────────────────────┤
│                      │  [网格] [列表]                    │
│                      │ ┌──────┐ ┌──────┐               │
...
子功能（标签筛选等）作为内容区顶部的筛选控件。存活状态仅在书签详情页展示，不作为列表筛选条件。
