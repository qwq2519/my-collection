...
- **添加 URL 时，若域名无对应站点则拒绝添加**，要求先收藏站点
...
├── subfolder/
│   ├── img3.jpg
│   └── .collections-cache    ← 每个文件夹一个缓存文件
└── .collections-cache        ← 当前文件夹的缓存文件
```
...
优势：
- 大部分文件不变时，比较根 hash 即可结束，跳过所有 readdir 调用
- 万级文件分布在多个子目录时，只进入有变化的子目录
- mtime + size 只读元数据，不读文件内容，极快

哈希树存储在缓存文件中，首次全量构建，后续仅对比和局部更新。
...
项目维护一个 `persist/` 文件夹，所有持久化数据集中存放：

```text
persist/
├── data.db              ← BuntDB 主数据文件
├── search.bleve/        ← Bleve 搜索索引
└── image-cache/         ← 图片缓存文件的汇总副本
```

### 备份方式

直接拷贝整个 `persist/` 文件夹即可完成备份和恢复。

### 图片缓存同步流程

```text
用户图片文件夹/.collections-cache
        │
        │ 文件变化时更新
        ▼
项目 persist/image-cache/
        │
        │ 应用启动或同步时读取
        ▼
内存（BuntDB + Bleve 索引）
