...
URL 标签（`url_tag`）和媒体标签（`media_tag`）是两套独立的标签体系，互不影响，但共享以下规则。详见 [标签系统](./tag-system.md)。

**存储规则**：大小写不敏感，存储时统一转小写。`::` 为逻辑层级分隔符，后端不解析层级关系。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| name | string | 是 | 标签字符串（小写），同 key 中的 `{name}`，规则见 [标签通用规则](#标签通用规则) |
...
当前无固定条目，后续可扩展（如数据版本号等）。

**索引脏状态判断**：不使用独立标志位，前端通过 `GetDirtyIndexStatus` 接口实时查询脏队列（`dirty:*` 前缀扫描）获取精确状态。
