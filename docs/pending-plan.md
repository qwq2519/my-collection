...
暂不实现数据迁移机制。media_meta.json 已有 `schema_version` 字段预留，后续需要时再设计 BuntDB 数据版本管理和迁移流程。
...
### ~~14. 图标存储的命名冲突~~ → 暂按 domain 维度

当前设计按 `{domain}.{ext}` 命名，站点和书签共享同域名图标。同一域名下只保留一个图标（域名级），暂不按实体 ID 分别存储。后续如有需要再扩展。
...
功能已移除，添加书签时若域名无对应站点直接拒绝并提示用户先创建站点。
